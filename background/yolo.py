...
def search_echoes_v2(img: np.ndarray, confidence_threshold=0.7, target_width=640) -> None | int:
    # 获取原始图像的尺寸
    h, w, _ = img.shape

    # 计算缩放比例，使得图像宽度为 target_width，保持比例
    scale = target_width / w
    new_w = target_width
    new_h = int(h * scale)  # 根据宽度的比例调整高度

    # 使用 NumPy 进行图像缩放
    resized_img = np.array([[img[int(y / scale), int(x / scale)] for x in range(new_w)] for y in range(new_h)], dtype=np.uint8)

    # 确保填充后的图像为正方形
    padded_img = np.full((target_width, target_width, 3), 114, dtype=np.uint8)  # 创建目标大小的背景图像

    # 计算纵向和横向的填充位置
    top = (target_width - new_h) // 2
    left = (target_width - new_w) // 2

    # 将缩放后的图像插入到填充图像中
    padded_img[top:top + new_h, left:left + new_w] = resized_img

    # 将图像转换为 (batch_size, channels, height, width) 格式，并归一化
    padded_img = padded_img.astype(np.float32) / 255.0  # 归一化到[0, 1]区间
    padded_img = np.transpose(padded_img, (2, 0, 1))  # 转换为 (3, h, w) 格式
    padded_img = np.expand_dims(padded_img, axis=0)  # 增加 batch 维度 (1, 3, h, w)

    # 准备输入
    inputs = {model.get_inputs()[0].name: padded_img}

    # 执行推理
    outputs = model.run(None, inputs)

    # 解析输出 (假设输出的第一个值是检测结果)
    boxes = outputs[0]  # 假设输出为 (batch_size, n, 6)，每个检测框包含 [x, y, w, h, confidence, class_id]
    ...
    # 获取每个目标的最大置信度，并找到对应的目标
    max_confidence_idx = np.argmax(boxes[0][:, 4])  # 找到最大置信度的索引
    max_confidence = boxes[0][max_confidence_idx, 4]  # 获取最大置信度

    # 只处理置信度最大的目标
    if max_confidence < confidence_threshold:
        print(f"最大置信度: {max_confidence:.2f}，未超过阈值，跳过目标筛选")
        return None

    print(f"最大置信度: {max_confidence:.2f}，超过阈值，进行目标筛选")
    x, y, w, h, conf, class_id = boxes[0][max_confidence_idx]
    print(f"最大置信度目标: 类别 {class_id}, 置信度 {conf:.2f}, 边界框: ({x}, {y}, {w}, {h})")
