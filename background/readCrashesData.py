...
#读取崩溃后的的值
def getCrashesValue():
    if os.path.exists("mc_log.txt"):  
        with open("mc_log.txt", "r", encoding="utf-8") as f:
            lines = f.readlines()
...
def isAppCrashes():
   if os.path.exists("isCrashes.txt"):  
        with open("isCrashes.txt", "r") as f:
        ...
        return value
   elif not os.path.exists("isCrashes.txt"):
...
def readCrashesDatas():
    isCrashes = isAppCrashes()
    if isCrashes: #游戏发生了崩溃-读取文本-True
           
            battle_count, absorb_count, heal_count = getCrashesValue() #读取崩溃后日志中保存的数据，作为日志输出
            return battle_count, absorb_count, heal_count
